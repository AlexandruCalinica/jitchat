defmodule Como.Documents do
  ...
  require Logger
  alias Como.Repo
  alias Como.Documents.{Document, RefDocument, DocumentWithLead}
  ...
  defp convert_md_to_lexical(md_content) do
    script_path =
      Application.app_dir(:como, "priv/scripts/convert_md_to_lexical")

    with {:ok, temp_path} <- Temp.path(%{suffix: ".md"}),
         :ok <- File.write(temp_path, md_content),
         {output, 0} <-
           System.cmd("sh", ["-c", "#{script_path} #{temp_path}"], stderr_to_stdout: true) do
      File.rm(temp_path)
      {:ok, output}
    else
      {error_output, _exit_code} ->
        Logger.error("Error converting markdown to lexical state: #{error_output}")
  ...
  defp initialize_y_writing(doc_id, lexical_state) do
    script_path =
      Application.app_dir(:como, "priv/scripts/convert_lexical_to_yjs")
    ...
    with {:ok, temp_path} <- Temp.path(%{suffix: ".json"}),
      ...
      File.rm(temp_path)
      Como.Documents.YDoc.insert_update(doc_id, output)
    else
      {error_output, _exit_code} ->
  ...
  def migrate_documents_lexical_state do
    ...
    Enum.each(documents, fn document ->
      case convert_md_to_lexical(document.body) do
        {:ok, lexical_state} ->
          Como.Documents.YDoc.clear_document(document.id)
          ...
          case initialize_y_writing(document.id, lexical_state) do
            ...
            {:error, error} ->
              Logger.error("Failed to initialize YDoc for document #{document.id}: #{error}")
          end
